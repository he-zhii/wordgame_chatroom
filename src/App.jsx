...
import {
  ...
  Coffee, Crown, Medal, ThumbsUp, Smile, AlertTriangle, MessageCircle, Flame, History
} from 'lucide-react';
import {
  UserProfileModal, DiscussionScreen, BrawlConfigModal, BrawlHistoryModal,
  USER_PROFILE_KEY, BRAWL_HISTORY_KEY, saveBrawlHistory
} from './NewFeatures';
...
export default function App() {
  ...
  const [showBrawlConfig, setShowBrawlConfig] = useState(false);
  const [showBrawlHistory, setShowBrawlHistory] = useState(false);
  const [brawlWords, setBrawlWords] = useState(null);
  const [brawlProgress, setBrawlProgress] = useState(null);
  ...
  useEffect(() => {
    ...
    // 加载大乱斗进度
    const storedBrawl = localStorage.getItem(KEYS.BRAWL);
    if (storedBrawl) {
      setBrawlProgress(JSON.parse(storedBrawl));
    }
  ...
  // 开始大乱斗
  const handleStartBrawl = (wordCount, continueGame) => {
    setShowBrawlConfig(false);

    if (continueGame && brawlProgress) {
      // 继续之前的游戏
      setBrawlWords(brawlProgress.words);
      setGameMode('brawl');
    } else {
      // 开始新游戏
      const allWords = Object.values(allWordsData).flat().filter(w => w.isActive !== false);
      const selectedWords = shuffleArrayFn(allWords).slice(0, wordCount);
      setBrawlWords(selectedWords);
      setBrawlProgress(null);
      clearBrawlProgress();
      setGameMode('brawl');
    }
  };
  ...
  // 大乱斗进度更新
  const handleBrawlProgressUpdate = (progress) => {
    setBrawlProgress(progress);
    saveBrawlProgress(progress);
  };

  // 大乱斗结束
  const handleBrawlEnd = () => {
    if (brawlProgress) {
      // 保存历史记录
      const record = {
        score: brawlProgress.score,
        correct: brawlProgress.currentIndex,
        total: brawlProgress.words.length,
        timestamp: Date.now()
      };
      saveBrawlHistory(record);
    }
    clearBrawlProgress();
    setBrawlProgress(null);
    setBrawlWords(null);
  ...
  const renderContent = () => {
    // 讨论区
    if (showDiscussion) {
      return <DiscussionScreen onBack={() => setShowDiscussion(false)} userProfile={userProfile || { avatar: '🐱', nickname: '匿名用户' }} />;
    }

    // 大乱斗模式
    if (gameMode === 'brawl' && brawlWords) {
      return (
        <GameScreen
          words={brawlWords}
          mode="brawl"
          onBack={handleBrawlEnd}
          settings={settings}
          onUpdateStats={handleUpdateStats}
          initialIndex={brawlProgress?.currentIndex || 0}
          initialScore={brawlProgress?.score || 0}
          preShuffled={true}
    ...
    return (
      <>
        ...
        <main className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
          ...
          {/* 大乱斗模式入口 */}
          <div
            onClick={() => setShowBrawlConfig(true)}
            className="group cursor-pointer rounded-3xl p-6 shadow-lg border-b-8 transition-all hover:-translate-y-2 hover:shadow-xl bg-gradient-to-br from-orange-400 to-red-500 border-red-600 active:scale-95"
          >
            <div className="flex justify-between items-start mb-4">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner bg-white/20">
                <Flame className="w-7 h-7 text-white" />
              </div>
              <button
                onClick={(e) => { e.stopPropagation(); setShowBrawlHistory(true); }}
                className="text-xs font-bold bg-white/20 text-white px-2 py-1 rounded-lg flex items-center gap-1 hover:bg-white/30"
              >
                <History className="w-3 h-3" /> 历史
              </button>
            </div>
            <h3 className="text-xl font-bold text-white">🔥 大乱斗</h3>
            <p className="text-white/80 text-sm font-medium mb-4">随机单词挑战</p>
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-white/20">
              <div className="flex gap-1 text-xs font-bold text-white/70">
                {brawlProgress ? `继续: ${brawlProgress.currentIndex + 1}/${brawlProgress.words.length}` : '全新挑战'}
              </div>
          ...
          <div
            ...
            className="group cursor-pointer rounded-3xl p-6 shadow-lg border-b-8 transition-all hover:-translate-y-2 hover:shadow-xl bg-gradient-to-br from-violet-400 to-pink-500 border-pink-600 active:scale-95"
          >
            <div className="flex justify-between items-start mb-4">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner bg-white/20">
                <MessageCircle className="w-7 h-7 text-white" />
              </div>
              <span className="text-xs font-bold bg-white/20 text-white px-2 py-1 rounded-lg">💬 互动</span>
            </div>
            <h3 className="text-xl font-bold text-white">讨论区</h3>
            <p className="text-white/80 text-sm font-medium mb-4">交流学习心得</p>
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-white/20">
              <div className="flex gap-1 text-xs font-bold text-white/70">分享 · 建议 · 讨论</div>
              <ArrowRight className="w-5 h-5 text-white/50 group-hover:text-white" />
            </div>
          </div>
        </main>
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-sky-50 font-sans pb-20">
      ...
      {/* 新功能模态框 */}
      <UserProfileModal isOpen={showUserProfileModal} onSave={handleSaveProfile} />
      <BrawlConfigModal
        isOpen={showBrawlConfig}
        onClose={() => setShowBrawlConfig(false)}
        onStart={handleStartBrawl}
        savedProgress={brawlProgress}
        allWordsData={allWordsData}
      />
      <BrawlHistoryModal isOpen={showBrawlHistory} onClose={() => setShowBrawlHistory(false)} />
    </div>
  );
}
